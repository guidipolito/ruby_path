...
class AboutRegularExpressions < Neo::Koan
  ...
  #
  # Why?
  # Because they take a lot of letters, I guess
  ...
  # An alternation you can access the elements that got matched in by indexes
  ...
  def test_sub_is_like_find_and_replace
    # Got the 1° word matched in the patterf of words preceding with T and replaced with the first letter
    # the first match, ( sub don't replace everthing )
    assert_equal 'one t-three', 'one two-three'.sub(/(t\w*)/) { ::Regexp.last_match(1)[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    # same than before, but gsub replace everthing
